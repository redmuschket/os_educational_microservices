...
Создан конфигурационный файл для домена `фамилия.ai-info.ru`. Активирован через симлинк, дефолтный сайт отключён.
...
Создана страница `feedback.html` с формой, содержащей поля «Имя» и «Сообщение», а также кнопку «Отправить». Форма отправляется методом POST на `/submit`.
...
Создана A-запись `api.фамилия.ai-info.ru`, указывающая на IP VPS.
...
- **Заголовок Host**: `Host: фамилия.ai-info.ru`
...
1. `Host: фамилия.ai-info.ru` > возвращает главную страницу Boardy
2. `Host: api.фамилия.ai-info.ru` > возвращает заглушку API
...
![Разные ответы по одному IP](screenshots/11-vhosts.png)
